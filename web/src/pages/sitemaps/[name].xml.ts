...
// No <lastmod> on venues/events: the pipeline bumps updated_at on every no-op
// upsert, so it would claim tens of thousands of changes per scrape.
...
import { MIN_CITY_EVENTS, eventChunk, parseSitemapName, urlsetXml, xmlResponse, type SitemapUrl } from '../../lib/sitemap.js';
...
export const GET: APIRoute = async ({ params }) => {
  ...
  if (parsed.name === 'static') urls = await staticUrls();
  else if (parsed.name === 'venues') urls = (await listSitemapVenues(db)).map((v) => ({ loc: `${SITE}/venue/${v.id}` }));
  else {
    ...
    urls = rows.map((e) => ({ loc: `${SITE}/event/${e.id}` }));
  }
